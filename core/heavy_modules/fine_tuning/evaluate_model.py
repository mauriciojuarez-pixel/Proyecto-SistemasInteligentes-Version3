...
def compute_metrics(pred):
    ...
    try:
        ...
        log_error(f"Error al calcular métricas: {e}")
        raise
...
def compare_with_baseline(new_results: dict, baseline_results: dict):
    ...
    try:
        ...
        log_info(f"Comparación con baseline: {comparison}")
        return comparison
    except Exception as e:
        log_error(f"Error al comparar con baseline: {e}")
        raise
...
def generate_evaluation_report(metrics: dict, output_file: str = "reports/evaluation/eval_results.json"):
    ...
    try:
        ...
        log_info(f"Reporte de evaluación generado: {output_file}")
    except Exception as e:
        log_error(f"Error al generar reporte de evaluación: {e}")
        raise
